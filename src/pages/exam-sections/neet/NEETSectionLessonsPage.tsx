...
interface NEETSectionLessonsPageProps {
  darkMode: boolean;
  setDarkMode: (value: boolean) => void;
}

const NEETSectionLessonsPage: React.FC<NEETSectionLessonsPageProps> = ({ darkMode, setDarkMode }) => {
  ...
  const checkEnrollment = async () => {
    try {
      const enrolled = await courseService.checkEnrollment(10);
  ...
  const fetchLessons = async () => {
    setIsLoading(true);
    try {
      console.log('🔍 Fetching lessons for NEET Biology course 10...');
      const allLessons = await courseService.getCourseLessons(10);
      ...
      const sectionData: Record<string, { title: string; icon: JSX.Element; color: string; description: string; lessonIds: number[] }> = {
        'botany': {
          ...
          icon: <FaLeaf />,
          color: '#10b981',
          description: 'Master Botany for NEET with comprehensive video lectures',
        ...
        'zoology': {
          ...
          icon: <FaPaw />,
          color: '#f59e0b',
          description: 'Master Zoology for NEET with detailed concept videos',
          lessonIds: [511, 512, 513, 514, 515, 516, 517, 518, 519, 520, 521, 522, 523, 524]
        },
        'biotech-genetics': {
          title: 'Biotechnology & Genetics',
          icon: <FaDna />,
          color: '#8b5cf6',
          description: 'Master Genetics & Biotechnology for NEET',
          lessonIds: [525, 526, 527, 528, 529, 530, 531, 532, 533, 534, 535, 536, 537]
        }
      };
  ...
  const handleStartLesson = (lessonId: number) => {
    navigate(`/courses/10/learn/${lessonId}`);
  };
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 relative z-10">
        ...
        <div className="mb-6">
          <button 
            ...
            onClick={() => navigate('/courses/10/sections')}
          >
...
export default NEETSectionLessonsPage;