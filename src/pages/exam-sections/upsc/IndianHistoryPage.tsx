...
const IndianHistoryPage: React.FC<IndianHistoryPageProps> = ({ darkMode, setDarkMode }) => {
  ...
  const fetchLessons = async () => {
    ...
    try {
      ...
      // Filter for Indian History lessons (IDs 801-810)
      const historyLessons = allLessons.filter(lesson => 
        lesson.id >= 801 && lesson.id <= 810
      );
      
      console.log('History lessons:', historyLessons);
      
      // Sort by ID
      historyLessons.sort((a, b) => a.id - b.id);
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 relative z-10">
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-3 mb-3">
            <FaLandmark className="text-5xl" style={{ color: '#3b82f6' }} />
            <h1 className="text-3xl font-bold text-gray-800 dark:text-white">Indian History</h1>
          </div>
          <p className="text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
            Complete Indian History for UPSC Prelims - Ancient, Medieval, and Modern India
          </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8 max-w-4xl mx-auto">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 flex items-center gap-4">
            <div className="w-14 h-14 rounded-xl flex items-center justify-center text-white text-2xl" style={{ background: '#3b82f6' }}>
              <FaBook />
            </div>
            <div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 max-w-4xl mx-auto">
          <div className="mb-4 text-sm text-gray-600 dark:text-gray-400">
            {lessons.length} lessons in Indian History
          </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <div className="text-xl mt-1" style={{ color: isLocked ? undefined : '#3b82f6' }}>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {lesson.isPreview && (
                          <span className="px-2 py-1 text-white text-xs font-semibold rounded-full whitespace-nowrap" style={{ background: '#3b82f6' }}>
                            FREE PREVIEW
                          </span>
                        )}
                      </div>
