...
const IndianPolityPage: React.FC<IndianPolityPageProps> = ({ darkMode, setDarkMode }) => {
  ...
  const fetchLessons = async () => {
    ...
    try {
      ...
      // Filter for Indian Polity lessons (IDs 816-820)
      const polityLessons = allLessons.filter(lesson => 
        lesson.id >= 816 && lesson.id <= 820
      );
      
      console.log('Polity lessons:', polityLessons);
      
      // Sort by ID
      polityLessons.sort((a, b) => a.id - b.id);
      
      setLessons(polityLessons);
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 relative z-10">
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-3 mb-3">
            <FaBalanceScale className="text-5xl" style={{ color: '#8b5cf6' }} />
            <h1 className="text-3xl font-bold text-gray-800 dark:text-white">Indian Polity</h1>
          </div>
          <p className="text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
            Complete Indian Polity for UPSC Prelims - Constitution, Fundamental Rights, Parliament, Judiciary
          </p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 max-w-4xl mx-auto">
          <div className="mb-4 text-sm text-gray-600 dark:text-gray-400">
            {lessons.length} lessons in Indian Polity
          </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <div className="text-xl mt-1" style={{ color: isLocked ? undefined : '#8b5cf6' }}>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {lesson.isPreview && (
                          <span className="px-2 py-1 text-white text-xs font-semibold rounded-full whitespace-nowrap" style={{ background: '#8b5cf6' }}>
                            FREE PREVIEW
                          </span>
                        )}
                      </div>
...
export default IndianPolityPage;